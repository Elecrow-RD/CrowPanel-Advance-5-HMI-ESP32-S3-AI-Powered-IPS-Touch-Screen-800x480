...
void buzzer_start() {
  // analogWrite(8, 249);
  // Start sending command 0x15 to address 0x30
  Wire.beginTransmission(0x30);            // 7-digit address 0x30
  Wire.write(0x15);                        // Send command 0x15
  uint8_t error = Wire.endTransmission();  // End transmission and return status

  if (error == 0) {
    Serial.println("Command 0x15 successfully sent");
  } else {
...
void buzzer_stop() {
  ...
  //  Start sending command 0x15 to address 0x30
  Wire.beginTransmission(0x30);            // 7-digit address 0x30
  Wire.write(0x16);                        // Send command 0x16
  uint8_t error = Wire.endTransmission();  // End transmission and return status

  if (error == 0) {
    Serial.println("Command 0x16 successfully sent");
  } else {
    Serial.print("Command sending error, error code：");
    Serial.println(error);
  }
